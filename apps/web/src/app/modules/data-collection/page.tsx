...
export default function DataCollectionPage() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      // Update data sources randomly
      setDataSources(prev => prev.map(source => ({
        ...source,
        recordCount: source.status === 'collecting' 
          ? source.recordCount + Math.floor(Math.random() * 50)
          : source.recordCount,
        lastUpdate: source.status === 'collecting' 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-green-900">{systemMetrics.usedStorage}</p>
                <p className="text-xs text-green-500 mt-1">of {systemMetrics.totalStorage}</p>
              </div>
        ...
        <Card className="border-orange-200 bg-orange-50/50">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-orange-600">System Load</p>
                <p className="text-3xl font-bold text-orange-900">{systemMetrics.systemLoad}%</p>
                <p className="text-xs text-orange-500 mt-1">{systemMetrics.activeConnections} connections</p>
              </div>
        ...
        <Card className="border-purple-200 bg-purple-50/50">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-purple-600">Data Rate</p>
                <p className="text-3xl font-bold text-purple-900">{systemMetrics.dataIngestionRate}</p>
                <p className="text-xs text-purple-500 mt-1">Network: {systemMetrics.networkThroughput}</p>
              </div>
        ...
        <CardContent>
          <div className={viewMode === 'grid' ? "grid grid-cols-1 lg:grid-cols-2 gap-4" : "space-y-4"}>
            {dataSources.map((source) => (
              <div 
                key={source.id} 
                className="flex items-center justify-between p-6 border rounded-lg hover:bg-muted/30 transition-all duration-200 cursor-pointer"
              ...
              >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 mt-3">
                      <div className="flex items-center gap-1">
                        ...
                        <span className={`text-sm font-medium ${getHealthScoreColor(source.healthScore)}`}>
                          {source.healthScore}%
                        </span>
                      </div>
                      <div className="flex items-center gap-1">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-sm font-medium">{(source.errorRate * 100).toFixed(3)}%</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    ...
                    <p className="text-sm font-medium">{source.lastUpdate}</p>
                  </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                <Progress 
                  ...
                  className="mb-1"
                />
                <p className="text-xs text-muted-foreground">{systemMetrics.systemLoad}% utilized</p>
              </div>
              <div>
                <Progress 
                  value={75} 
                  ...
                  className="mb-1"
                />
                <p className="text-xs text-muted-foreground">1.8TB of 2.4TB used</p>
              </div>
              <div>
                <Progress 
                  value={60} 
                  ...
                  className="mb-1"
                />
                <p className="text-xs text-muted-foreground">{systemMetrics.networkThroughput} active</p>
              </div>
              <div>
                <Progress 
                  value={5} 
                  ...
                  className="mb-1"
                />
                <p className="text-xs text-muted-foreground">0.12% average error rate</p>
              </div>
            </div>
          </CardContent>
        </Card>
